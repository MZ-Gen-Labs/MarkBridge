...
- **Image Export**: 画像エクスポートモード
  ...
  - Files: 外部ファイルとして保存（ベース名フォルダに配置）

> **画像出力フォルダ仕様:** Filesモードでは、出力Markdownファイルのベース名でフォルダを作成し、そこに画像を保存する。
> 例: `output.md` → `output/table_0.png`, `output/table_1.png`
...
プレビュー表示時、相対パスの画像をBase64データURIに変換してWebViewで表示。
- Markdownファイル自体は相対パスのまま保存
- プレビュー時のみメモリ内でBase64変換
- ファイルサイズへの影響なし
...
Tools/
└── Setup-DoclingVenv.ps1  # 環境構築PowerShellスクリプト
```
...
import docling_convert  # 本体スクリプト
```

**環境構築スクリプト:**

`Tools/Setup-DoclingVenv.ps1`は本番・テスト両方の環境構築に使用可能:

```powershell
# 公式版Doclingインストール
.\Tools\Setup-DoclingVenv.ps1 -VenvPath "C:\venv\docling" -PythonPath "py -3.12" -Mode nightly

# フォーク版Doclingインストール
.\Tools\Setup-DoclingVenv.ps1 -VenvPath "C:\venv\docling" -PythonPath "py -3.12" -Mode nightly -UseForkedDocling
```
